...
| $C0 im2/nmi | $00 | $00 | **NOT WIRED** | reset value conforms, but NR$C0 is the IM2 vector base and the IM2 daisy chain is not connected to the CPU's interrupt path: the register is stored and never acted on. Same for NR$CC/$CD/$CE. The CTC itself is wired (ports $183B..$1F3B); only its interrupts are undelivered. ROADMAP item 2 |
