...
| Where Time Stood Still (+3 disk reissue) | Known issue | Re-checked 2026-08-14. Same fault as the rows below: 18432 px in 2 colours is the uniform stripe pattern of uninitialised video memory, not a title screen, and a differential run reports both machines blank. The "answered a keypress" was measured on that same non-screen, so it is not evidence about the title either. |
...
| California Games (+3 disk) | Known issue | Re-checked 2026-08-14, and the **Boots (responds)** verdict was wrong: the 5973 px in 7 colours it was scored on is the +3 ROM menu, which this file's own screening notes say to read as "never left the menu". A differential run gets past the menu but draws almost nothing — 632 lit pixels, a black screen with one blue bar — and the **reference never leaves its menu at all** on this image, so the comparison voids as LOADING and cannot say whose fault it is. A MAME run was attempted as a third opinion and is **inconclusive**: its image mounts, but the ENTER that selects Loader did not register, so it sat on the +3 menu and never started the load. The image itself is structurally sound — 42 cylinders, a valid 512-byte boot sector checksumming to 3. **Still unresolved, and now precisely characterised** by `ZX_GO_FDC_TRACE=1`: the loader reads its data cylinders 36 and 37 normally, recalibrates, seeks to **cylinder 7 — a protection track whose sectors are numbered $B1-$B8** — issues exactly one READ DIAGNOSTIC asking for R=01, and that is the last command it ever sends. Tracing found a real bug there (READ A TRACK was not setting ND when the ID it was given is absent, fixed with the datasheet's wording behind it), but **that fix did not change this title**: the run is byte-for-byte what it was, 632 lit pixels. So what the loader wants from that protection track is still unknown. |
