...
- **DISCiPLE / +D / disk drives** — enable, then File → Load Disk / DISCiPLE
  ...
  `.dsk` images load, including the EXTENDED variant, disks whose tracks pack
  more sectors than the nominal gap layout allows, and copy-protected titles
  whose tracks declare more sector data than a physical double-density track
  can hold (a size code claiming 8192 bytes against roughly 6250). The
  controller reads those the way the real one does: it takes the declared size
  at face value and streams across the index hole to get it.

  Of a 250-image sample, one fails to parse, and that one is a truncated dump
  whose track data runs past the end of the file.
