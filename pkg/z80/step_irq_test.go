...
func TestStepPlacesTheNextBoundaryAMachineFrameLater(t *testing.T) {
	...
	// IFF1 stays false so IRQPending is observable rather than being
	// accepted and cleared inside the same step, matching
	// TestStepInstructionWithIRQ_FiresAtFrameBoundary above.
	c.IFF1 = false
	...
	// The default budget would have put the boundary at a multiple of
	// 70908; this is the assertion that distinguishes the two.
	if c.nextFrameBoundary%stepFrameBudget == 0 {
		t.Errorf("next boundary %d is also a multiple of the default budget: the test cannot tell them apart",
			c.nextFrameBoundary)
	}
}
